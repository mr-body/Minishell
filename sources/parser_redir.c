...
/*   Updated: 2024/12/09 14:43:40 by gkomba           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    if (vars->current_arg[0] != '\0') // Se o argumento atual não estiver vazio
    {
        if (vars->is_redirect) // Se for um redirecionador
        {
            vars->data_idx++;
            data[vars->data_idx] = malloc(ft_strlen(vars->current_arg) + 1);
            if (data[vars->data_idx]) // Copia o argumento para data
                ft_strcpy(data[vars->data_idx], vars->current_arg);
            vars->is_redirect = false; // Reseta o estado de redirecionador
        }
        else // Caso contrário, concatena ao comando
        {
            ft_strcat_no_malloc(data[0], vars->current_arg);
            ft_strcat_no_malloc(data[0], " ");
        }
        vars->current_arg[0] = '\0'; // Limpa o argumento atual
    }
}
...
static void split_redir(t_split_redir_cmd *vars, char *command, char **data)
{
    while (command[vars->i] != '\0')
    {
        ...
        {
            finalize_argument(vars, data); // Finaliza o argumento atual
            vars->is_redirect = true; // Marca como redirecionador
            vars->i++; // Move o índice para o próximo caractere
            continue;
        }
        if ((!vars->in_s_quotes && !vars->in_d_quotes)
            && command[vars->i] == ' ')
